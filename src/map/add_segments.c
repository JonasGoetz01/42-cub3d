...
/*   Updated: 2024/07/22 14:33:47 by cgerling         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_wall_segments(t_global *global, t_line **lines, int *line_count,
			t_add_segments *seg)
{
	if (!is_wall_above(global, seg->x, seg->y)
		&& !is_door_above(global, seg->x, seg->y))
	{
		add_line_segment(lines, line_count, set_wall_segment(seg->top_left,
				seg->top_right, HORIZONTAL));
	}
	if (!is_wall_below(global, seg->x, seg->y)
		&& !is_door_below(global, seg->x, seg->y))
	{
		add_line_segment(lines, line_count, set_wall_segment(seg->bottom_left,
				seg->bottom_right, HORIZONTAL));
	}
	if (!is_wall_left(global, seg->x, seg->y)
		&& !is_door_left(global, seg->x, seg->y))
	{
		add_line_segment(lines, line_count, set_wall_segment(seg->top_left,
				seg->bottom_left, VERTICAL));
	}
	if (!is_wall_right(global, seg->x, seg->y)
		&& !is_door_right(global, seg->x, seg->y))
	{
			...
			t_add_segments *seg)
{
	...
	add_line_segment(lines, line_count, set_door_side_segment(seg->top_left,
			seg->top_right, VERTICAL, seg->door));
			...
			t_add_segments *seg)
{
	...
	if (!seg->door)
		ft_exit_free(global);
	if (is_wall_left(global, seg->x, seg->y)
		&& is_wall_right(global, seg->x, seg->y))
	{
